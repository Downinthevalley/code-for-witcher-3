...
statemachine abstract import class CR4Player extends CPlayer
{
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		LogChannel('ModSpawnCompanions', "SCMEntity2: " + mod_scm_GetSCMEntity());
	...
	public function mod_scm_CreateSCM() : mod_scm
	{
		if(!MODSCM)
		{
	...
	timer function mod_scm_4second( dt : float, id : int )
	{
	...
	public function FistFightHealthChange( val : bool )
	{
		...
		if( val == true )
		{
			...
			{
				ClampGeraltMaxHealth( GeraltMaxHealth-GeraltMaxHealth*0.55 );
				SetHealthPerc( 100 );
			}
			else if (diffMode == EDM_Hard)
			{
				ClampGeraltMaxHealth( GeraltMaxHealth-GeraltMaxHealth*0.35 );
				SetHealthPerc( 100 );
			}
			...
			{
				ClampGeraltMaxHealth( GeraltMaxHealth/4 );
				SetHealthPerc( 100 );
			}
		}
		// modImprovedFistFightNGE
	...
	{
		var slowmoCam : CCustomCamera = theGame.GetGameCamera(); //Critical Slow Motion Combat Redux
		...
		//Critical Slow Motion Combat Redux	
		if ( mCSMCR.critSloMoToggle && mCSMCR.slowOnDead != 3 && isSlowDeathTimer  )
		{
			theGame.GetGameCamera().ChangePivotRotationController( 'Default' );
			...
			moveData.pivotRotationController.SetDesiredPitch(-18.0f);
			...
			moveData.pivotRotationValue.Pitch	= LerpAngleF( -2.0f * dt, moveData.pivotRotationValue.Pitch, targetRotation.Pitch );
			moveData.pivotRotationController.minPitch = -18.0f;

			moveData.pivotPositionController.offsetZ = -0.6f;
			//slowmoCam.fov = 30;
			return true;
		}
		//Critical Slow Motion Combat Redux
		...
		if ( theGame.IsFocusModeActive() && !gmConfig().enableSenseNoZoom ) // ===== Added by God Mode mod - Module: Sense No Zoom =====
		{
	...
	public function CanSprint( speed : float ) : bool
	{
		...
		if(gmConfig().enableNoEncumbered)	{	GetWitcherPlayer().RemoveAllBuffsOfType( EET_OverEncumbered );	}  // ===== Added by God Mode mod - Module: No Encumbered =====
		...
		if( theGame.IsFocusModeActive() && !gmConfig().enableSenseJumpRun )  // ===== Added by God Mode mod - Module: Sense Run Jump =====
		{
			return false;
		}
	...
	public function UpdateDisplayTarget( optional forceUpdate : bool, optional forceNullActor : bool )
	{
		...
		//Critical Slow Motion Combat Redux	
		if( GetCurrentlyCastSign() == ST_Aard && (GetBehaviorVariable( 'combatActionType' ) == (int)CAT_CastSign) && IsInCombat() && !slowActive && mCSMCR.critSloMoToggle && mCSMCR.aardSweepOnly != 3 )
		{
			slowActive = true;
			AddTimer('aardSlowTimer',0.25f, false);
			AddTimer('DeactivateSlowMoCam',0.8f, false);
		}
		if( GetCurrentlyCastSign() == ST_Igni && (GetBehaviorVariable( 'combatActionType' ) == (int)CAT_CastSign) && IsInCombat() && !slowActive && mCSMCR.critSloMoToggle && mCSMCR.igniNormalOnly != 3 )  
		{
			slowActive = true;
			AddTimer('igniSlowTimer',0.25f, false);
			AddTimer('DeactivateSlowMoCam',0.8f, false);
		}
		//Critical Slow Motion Combat Redux
	...
	function InitTargeting()
	{
		...
		if (!theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETVersion'))
		{
			...
			theGame.GetInGameConfigWrapper().SetVarValue( 'EnhancedTargeting', 'ETDistanceWeight', FloatToString(1.0f));
			...
			theGame.GetInGameConfigWrapper().SetVarValue( 'EnhancedTargeting', 'ETVersion', "3.0");
		}
		else if (StringToFloat(theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETVersion')) < 3.0)
		{
			theGame.GetInGameConfigWrapper().SetVarValue( 'EnhancedTargeting', 'ETVersion', "3.0");
		}
	...
	protected function FindTarget( optional actionCheck : bool, optional action : EBufferActionType, optional actionInput : bool ) : CActor
	{
		...
		//modEnhancedTargeting BEGIN
		if ( IsInCombat() && ( !theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETVersion') || theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETEnabled') ) )
		{
			FindTargetSimple();
		}
		else
	...
	protected function FindTargetSimple()
	{
		...
		if ( igconfig.GetVarValue('EnhancedTargeting', 'ETVersion') )
		{
			cameraWeight = StringToFloat(igconfig.GetVarValue('EnhancedTargeting', 'ETCameraWeight'));
			movementWeight = StringToFloat(igconfig.GetVarValue('EnhancedTargeting', 'ETMovementWeight'));
			facingWeight = StringToFloat(igconfig.GetVarValue('EnhancedTargeting', 'ETFacingWeight'));
			distanceWeight = StringToFloat(igconfig.GetVarValue('EnhancedTargeting', 'ETDistanceWeight'));
			targetHysteresis = StringToFloat(igconfig.GetVarValue('EnhancedTargeting', 'ETHysteresis'));
			targetingInfo.inFrameCheck = igconfig.GetVarValue('EnhancedTargeting', 'ETInFrameCheck');
		}
		else
		{
			cameraWeight = 1.0f;
			movementWeight = 0.75f;
			facingWeight = 0.25f;
			distanceWeight = 1.0f;
			targetHysteresis = 1.05f;
			targetingInfo.inFrameCheck = false;
		}
		...
		// change distance based on situation
		if ( IsSwimming() )
			targetingInfo.coneDist = theGame.params.MAX_THROW_RANGE;
		else
			targetingInfo.coneDist = softLockDist;
		
		// check if locked on to target and if need to clear lock because target is too far
		...
		// check if blocking buffs prevent finding target or if `bCanFindTarget` is disabled
		if ( ( thePlayer.HasBuff( EET_Confusion ) || thePlayer.HasBuff( EET_Hypnotized ) || thePlayer.HasBuff( EET_Blindness ) || thePlayer.HasBuff( EET_WraithBlindness ) ) || !bCanFindTarget )
			return;
		
		// check if aiming and grab the aimed at target, if it exists
		if ( this.playerAiming.GetCurrentStateName() == 'Aiming' )
		{
			newTarget = this.playerAiming.GetAimedTarget();
		...
		// get a list of visible targets
		GetVisibleEnemies( targets );
		
		// add finishableEnemies if they aren't in
		for ( i = 0; i < finishableEnemiesList.Size(); i += 1 )
		{
			if ( !targets.Contains( finishableEnemiesList[i] ) )
			{
		...
		// filter list of targets to only contain viable targets
		for ( i = targets.Size() - 1; i >= 0; i -= 1 )
		{
			targetingInfo.targetEntity = targets[i];
		...
		if ( targets.Size() > 0 )
		{
			...
			for ( i = 0; i < targets.Size(); i += 1 )
			{
				...
				{
					// modify hysteresis if in an attack
			...
			if ( newTarget )
			{
				...
				{
					//theGame.witcherLog.AddMessage("New softlock target: " + newTarget.GetDisplayName());
					//theGame.witcherLog.AddMessage("num targets: " + precull);
					//theGame.witcherLog.AddMessage("num valid targets: " + targets.Size());
					//theGame.witcherLog.AddMessage("old rank: " + curTargetRank);
					//theGame.witcherLog.AddMessage("target rank: " + bestRank);
	...
	public function LockToTarget( flag : bool )
	{
		...
		//modEnhancedTargeting BEGIN
		if (theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETVersion') && theGame.GetInGameConfigWrapper().GetVarValue('EnhancedTargeting', 'ETDisableCameraLock'))
		{
			if (IsCameraLockedToTarget())
	...
	protected function PlayHitAnimation(damageAction : W3DamageAction, animType : EHitReactionType)
	{
		...
		//Critical Slow Motion Combat Redux
		if ( mCSMCR.critSloMoToggle && slowActive && IsAlive() )
		{
			//theGame.GetGuiManager().ShowNotification( "Damage");
			if ( mCSMCR.slowSFX == 0 ) {
				if(!theSound.SoundIsBankLoaded('magic_djinn.bnk'))
					theSound.SoundLoadBank('magic_djinn.bnk', false);
				theSound.SoundEvent('magic_djinn_vfx_glow_loop_stop');
			...
			theGame.RemoveTimeScale(theGame.GetTimescaleSource(8));
			GetWitcherPlayer().SetCurrentlyCastSign(ST_None,GetWitcherPlayer().GetCurrentSignEntity());
			slowActive = false;
			//AddTimer('DeactivateSlowMoCam', 0.1);	
		}
		//Critical Slow Motion Combat Redux
	...
	public function ReactToBeingHit(damageAction : W3DamageAction, optional buffNotApplied : bool) : bool
	{
		...
		{
		    if( !((W3Effect_Toxicity)damageAction.causer) ) //modDUI_CrossbowFix_NextGen
			OnRangedForceHolster( true, true );
		}
	...
	{
		if(gmConfig().enableNoItemLevel)	{	return true;	}		// ===== Added by God Mode mod - Module: No Item Level =====
		if (thePlayer.IsCiri())
			return true;
		//New Game Plus Plus begin
		if (theGame.GetInGameConfigWrapper().GetVarValue('NewGamePlusPlus', 'NoEquipmentLevelRequirement'))
			return true;
		//New Game Plus Plus end
	...
	{	
		...
		{
			if ( GetBehaviorVariable( 'combatActionType' ) == (int)CAT_CastSign )
				if (gmConfig().keepTorches) {holsterUsableItem = false;} else {holsterUsableItem = true;} // ===== Added by God Mode mod - Module: Misc Keep Torches =====
	...
	private function PlayFinisherCameraAnimation( cameraAnimName : name )
	{
        ...
        // ===== Added by God Mode mod - Module: Gore - Finisher =====
        if (gmConfig().goremodefinish > 0) {
		if( IsLastEnemyKilled() )
		{
			camera.StopAnimation('camera_shake_hit_lvl3_1' );
	...
	} else if( IsLastEnemyKilled() && theGame.GetWorld().NavigationCircleTest( this.GetWorldPosition(), 3.f ) )
		{
			camera.StopAnimation('camera_shake_hit_lvl3_1' );
			...
			{
				animation.blendIn = mCSMCR.dismemberSlowMoFactor;
				slowActive = false;
				theGame.RemoveTimeScale(theGame.GetTimescaleSource(8));	
			}
			//Critical Slow Motion Combat Redux
	...
	public function IsLastEnemyKilled() : bool
	{
		...
		if ( mCSMCR.critSloMoToggle )
		{
			if (mCSMCR.finisherCamToggle == 0)
				return true;
			else if (mCSMCR.finisherCamToggle == 2)
				return false; 
		}
		//Critical Slow Motion Combat Redux	
	...
	event OnCombatActionStartVehicle( action : EVehicleCombatAction )
	{
		...
		if ( action != EHCA_ShootCrossbow )
		{
			...
			UnblockAction( EIAB_Crossbow, 'OnForceHolster' ); //modDUI_CrossbowFix_NextGen
		}		
	}
	...
	public function CheatResurrect()
	{
		...
		var hud : CR4ScriptedHud; //Critical Slow Motion Combat Redux
	...
	event OnEnterShallowWater()
	{
		...
		if(!gmConfig().enableJumpSprint)	{	BlockAction( EIAB_Sprint,'ShallowWater', false, false, true );	} // ===== Added by God Mode mod - Module: Movement Jump Sprint =====
...
/*  
           ...
           // ===== Added by God Mode mod - Module: Misc No Borders =====
           if (!gmConfig().noBorders) {	
